...
class ViCLIP(nn.Module):
    ...
    def forward(self, image, raw_text, return_sims=False):
        """forward and calculate loss.

        Args:
            image (torch.Tensor): The input images. Shape: [B,T,C,H,W].
            raw_text (List[str]): The textual prompts.

        Returns:
            similarities, is return_sims=True,
            dictionary with loss value otherwise

        """
        self.clip_contrastive_temperature()

        # if return_sims=True, we want no masking
        vision_embeds = self.encode_vision(image, test=return_sims)
